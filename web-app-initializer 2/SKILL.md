...
4. **Generated `docs/` files**: `app-overview.md`, `ux-decisions.md`, `feature-roadmap.md` (each generated from confirmed Phase 3-5 content), plus `working-notes.md` (copied from template — empty sections for ongoing project memory)
...
6. Copy `templates/docs/working-notes.md` to the project's `docs/working-notes.md` as-is. Empty append-only log for known bugs, dead ends, gotchas, decisions, and prototype overrides.
7. Create route folders under `app/` matching the confirmed route map
8. For each route:
...
9. Apply naming rules from `references/naming.md` to every file generated
10. Run `git init && git add -A && git commit -m "Initial scaffold from prototype"`
11. Print next steps:
